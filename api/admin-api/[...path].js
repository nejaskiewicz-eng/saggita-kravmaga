...
/**
 * Vercel catch-all czasem nie podaje req.query.path,
...
// Bezpieczne liczby do LIMIT/OFFSET
  ...
  try {
    ...
    if (raw === "/status" && method === "GET") {
      ...
      return res.status(200).json({
        ...
        server_time: now,
        admin_users: cnt,
        note: cnt > 0 ? "Admin istnieje → użyj /login." : "Brak admina → zrób /setup (POST).",
      });
    }
    ...
    if (raw === "/login" && method === "POST") {
      ...
      const { rows } = await pool.query("SELECT * FROM admin_users WHERE username = $1", [
        String(username).trim(),
      ]);
    ...
    // ── GET /stats ───────────────────────────────────────────────
    // ✅ nie używamy v_stats (bo u Ciebie go nie ma) – liczymy staty "w locie"
