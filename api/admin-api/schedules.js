...
const PL_DAYS = {
  ...
  5: 'Piątek', 6: 'Sobota', 0: 'Niedziela', 7: 'Niedziela',
};
  ...
  if (req.method === 'GET') {
    ...
    try {
      ...
      const { rows: schedules } = await pool.query(`
        SELECT id, group_id, day_of_week, day_name, time_start,
               time_end, time_label, address, active
        FROM schedules
      ...
      for (const g of groups) {
        ...
        const groupObj = {
          id: g.id, location_id: g.location_id, name: g.name,
          category: g.category, age_range: g.age_range,
          max_capacity: maxCap, notes: g.notes, active: g.active,
          registered: reg, available, schedules: [],
        };
      ...
      for (const s of schedules) {
        ...
        g.schedules.push({
          ...
          day_of_week: s.day_of_week, day_name: dayName,
          time_start: s.time_start, time_end: s.time_end,
          time_label: timeLabel, address: s.address || null,
        });
      }
      ...
      const result = Array.from(locMap.values()).map(loc => ({
        ...
        groups: (loc.groups || [])
          ...
          .map(gr => ({ ...gr, schedules: gr.schedules || [] })),
      }));
    ...
    try {
      ...
      const { rows: [s] } = await pool.query(
        `INSERT INTO schedules (group_id, day_of_week, day_name, time_start, time_end, time_label, address, active)
         ...
         RETURNING *`,
        [
          b.group_id, b.day_of_week, day_name,
