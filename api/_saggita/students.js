...
// FIX: kosmiczne kwoty (multiplikacja przez JOINy) → agregaty płatności liczone w LATERAL
// DODANE: filtr sezonu 2025/2026 przez ?season=2025 (01.09.2025–23.02.2026)
...
module.exports = async (req, res) => {
  ...
  const pool = getPool();
  const {
    id, pid, aid, _route, search, source, group_id, city,
    payment_status, is_active, page = 1, limit = 60, sort = 'recent',
    season
  } = req.query;
  ...
  if (id && _route === 'payments') {
    if (req.method === 'GET') {
      ...
      const { rows: regs } = await pool.query(
        `SELECT r.id,'registration' AS type,r.total_amount AS amount,r.created_at AS date,r.admin_notes AS note,
                r.payment_ref,r.payment_status AS status,pp.name AS plan_name
         FROM students s
    ...
    if (req.method === 'PATCH' && pid) {
      ...
      vals.push(pid, id);
      try {
        await pool.query(`UPDATE legacy_payments SET ${set.join(',')} WHERE id=$${pi} AND student_id=$${pi + 1}`, vals);
    ...
    if (req.method === 'GET') {
      try {
        const { rows } = await pool.query(
          `SELECT a.id,a.present,a.diff_group,ts.session_date,ts.id AS session_id,g.name AS group_name
           FROM attendances a
    ...
    try {
      ...
      if (student.registration_id) {
        const { rows: [r] } = await pool.query(
          `SELECT r.*,g.name AS group_name,l.city,pp.name AS plan_name
  ...
  if (req.method === 'GET') {
    try {
      ...
      if (city) {
        conds.push(`EXISTS(
          SELECT 1 FROM student_groups sg3
      ...
      // ─── FILTR SEZONU (01.09.2025–23.02.2026) ───
      if (season === '2025') {
        conds.push(`
          EXISTS (
            SELECT 1
            FROM attendances aS
            JOIN training_sessions tsS ON tsS.id = aS.session_id
            WHERE aS.student_id = s.id
              AND tsS.session_date >= DATE '2025-09-01'
              AND tsS.session_date <= DATE '2026-02-23'
          )
        `);
      }
      ...
      const { rows } = await pool.query(`
        SELECT
          s.id,s.legacy_id,s.first_name,s.last_name,s.email,s.phone,s.birth_year,s.is_active,s.source,s.created_at,s.registration_id,
          ...
          CASE WHEN pay.last_payment_date IS NOT NULL THEN (CURRENT_DATE - pay.last_payment_date::date)::int ELSE NULL END AS days_since_payment,

          r.payment_status,r.total_amount,pp.name AS plan_name,
          l.city
        ...
        -- FIX: agregaty płatności liczymy osobno (bez multiplikacji przez JOINy)
        LEFT JOIN LATERAL (
          SELECT
        ...
        GROUP BY s.id, pay.legacy_paid, pay.last_payment_date, r.payment_status, r.total_amount, pp.name, l.city

        ORDER BY ${
          sort === 'alpha'
            ? 's.last_name, s.first_name'
            : sort === 'payment'
              ? 'pay.last_payment_date DESC NULLS LAST, s.last_name, s.first_name'
              : 'MAX(ts.session_date) DESC NULLS LAST, pay.last_payment_date DESC NULLS LAST, s.last_name, s.first_name'
        }
  ...
  if (req.method === 'PATCH' && id) {
    ...
    if (!set.length) return res.status(400).json({ error: 'Brak pól.' });
    vals.push(id);
    try { await pool.query(`UPDATE students SET ${set.join(',')} WHERE id=$${pi2}`, vals); return res.status(200).json({ success: true }); }
    catch (e) { return res.status(500).json({ error: e.message }); }
  }
  ...
  if (req.method === 'DELETE' && id) {
    try {
      const { rows: [{ att, pay }] } = await pool.query(
        `SELECT
